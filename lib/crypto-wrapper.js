...
CryptoWrapper.prototype.UUID = function() {
	var self = this;
...
CryptoWrapper.prototype.GenerateKey = function() {
   ...
   var key = uuid.v4();
   key = key.replace(/\-/g, CryptoWrapper.RandomString(1));
...
CryptoWrapper.prototype.GenerateSalt = function() {
   ...
   var salt = uuid.v4();
   salt = salt.replace(/\-/g, CryptoWrapper.RandomString(1));
...
CryptoWrapper.prototype.Sign = function(data) {
   ...
   return crypto
            .createSign(self.options.signer)
            .update(data)
            .sign( fs.readFileSync(self.options.private_key_file), self.options.format );
};
...
CryptoWrapper.prototype.Verify = function(data, signature) {
   ...
   return crypto
            .createVerify(self.options.signer)
            .update(data)
            .verify( fs.readFileSync(self.options.public_key_file), signature, self.options.format );
};
...
CryptoWrapper.prototype.VerifyHash = function(data, hash) {
   ...
   // HashCrypto
   // return CryptoWrapper.HashCrypto(data) === hash;
...
CryptoWrapper.prototype.Hash = function(data, iterations) {
   ...
   // return CryptoWrapper.HashCrypto(data, iterations);
   return CryptoWrapper.HashBcrypt(data, iterations);
};
...
CryptoWrapper.prototype.Encrypt = function(data, callback) {
   ...
   CryptoWrapper.GenerateIV(self.options.iv_size, function(err, iv) {
      if (err) {
         return callback(err);
      }

      CryptoWrapper.Cipher(data, self.options.secret_key, self.options.salt, iv, function(err, ciphertext) {
         if (err) {
            return callback(err);
         }

         callback(null, CryptoWrapper.Pack({
...
/*
 * Crypt.Decrypt()
 * @param buffer packedBuffer - must be properly packed using CryptoWrapper.Pack()
 */
CryptoWrapper.prototype.Decrypt = function(packedBuffer, callback) {
	var self = this;

   var unpackedObj = CryptoWrapper.Unpack(new Buffer(packedBuffer, self.options.format));
   ...
   CryptoWrapper.Decipher(ciphertext, self.options.secret_key, self.options.salt, iv, function(err, plaintext) {
      if (err) {
         return callback(err);
      }
