...
CryptoWrapper.prototype.Cipher100 = function(plaintext, secret_key, iv, callback) {
	...
	try {
		scrypt.kdf(new Buffer(secret_key), self.options.scrypt_params, self.options.key_size + self.options.mac_size, function(err, obj) {
			if (err) {
				return callback(err);
			}

			var kdf_hash_buffer = obj.hash;

			var hashed_key_buffer = new Buffer(self.options.key_size);
			var mac_tag_buffer = new Buffer(self.options.mac_size);

			kdf_hash_buffer.copy(hashed_key_buffer, 0, 0, self.options.key_size);
			kdf_hash_buffer.copy(mac_tag_buffer, 0, self.options.key_size, self.options.mac_size);

			try {
				var cipher = crypto.createCipheriv(self.options.cipher_algorithm, hashed_key_buffer, iv);
				...
				callback(null, { iv: iv, ciphertext: ciphertext, mac: mac_tag_buffer, hashed_key: hashed_key_buffer });
			} catch(e) {
				return callback(e);
			}
...
// TODO: Implement MAC
CryptoWrapper.prototype.Decipher100 = function(ciphertext, hashed_key_buffer, iv, callback) {
	var self = this;

	try {
		var decipher = crypto.createDecipheriv(self.options.cipher_algorithm, hashed_key_buffer, iv);
