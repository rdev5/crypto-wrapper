...
CryptoWrapper.prototype.Decipher100 = function(ciphertext_obj, mac_obj, callback) {
	var self = this;

	try {
		var iv = new Buffer(ciphertext_obj.iv, self.options.format);
		var ciphertext = new Buffer(ciphertext_obj.ciphertext, self.options.format);
		var hashed_key = new Buffer(ciphertext_obj.hashed_key, self.options.format);

		var mactag = new Buffer(mac_obj.mactag, self.options.format);
		var mac_key = new Buffer(mac_obj.mac_key, self.options.format);
