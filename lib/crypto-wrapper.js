...
var default_options = {
   ...
   signer_algorithm: 'sha1',
	...
	// See https://github.com/barrysteyn/node-scrypt#kdf-1
	scrypt_params: { N: 16384, r: 8, p: 1 },

	scrypt_kdf_config: {
		saltEncoding: 'buffer',
		keyEncoding: 'ascii',
		outputEncoding: 'buffer', // must remain as buffer
		defaultSaltSize: 256,
	...
	scrypt_verify_config: {
		hashEncoding: 'hex', // should match scrypt_hash_config.outputEncoding
		keyEncoding: 'ascii'
	}
};
...
CryptoWrapper.prototype.DevRandom = function(size, callback) {
	var self = this;
...
CryptoWrapper.prototype.Hash100 = function(plaintext, secret_key, callback) {
	var self = this;

	try {
		_.assign(scrypt.hash.config, self.options.scrypt_hash_config);

		scrypt.hash(secret_key, self.options.scrypt_params, function(err, hash) {
			if (err) {
				return callback(err);
			}
...
CryptoWrapper.prototype.VerifyHash100 = function(hash, secret_key, callback) {
	var self = this;

	try {
		_.assign(scrypt.verify.config, self.options.scrypt_verify_config);

		scrypt.verify(hash, secret_key, function(err, verified) {
			if (err) {
...
CryptoWrapper.prototype.GenerateSalt020 = function(salt_rounds, seed_length) {
	var self = this;
...
CryptoWrapper.prototype.Hash020 = function(data, salt_rounds, seed_length) {
	var self = this;
	
   return bcrypt.hashSync( data, self.GenerateSalt020(salt_rounds, seed_length) );
};

CryptoWrapper.prototype.VerifyHash020 = function(data, hash) {
	var self = this;
...
CryptoWrapper.prototype.Cipher020 = function(plaintext, secret_key, salt, iv, callback) {
	...
	if (!salt) salt = self.GenerateSalt020();
 ...
 */
CryptoWrapper.prototype.Pack020 = function(unpackedObj) {
	var self = this;
 ...
 */
CryptoWrapper.prototype.Unpack020 = function(packedBuffer) {
	var self = this;
...
CryptoWrapper.prototype.Decrypt020 = function(packedBuffer, secret_key, salt, callback) {
   ...
   var unpackedObj = self.Unpack020(new Buffer(packedBuffer, self.options.format));
   if (!unpackedObj) {
